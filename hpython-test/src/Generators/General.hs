...
genParam :: MonadGen m => m (Expr '[] ()) -> m (Param Expr '[] ())
genParam genExpr =
  sizedRecursive
    [ StarParam (Ann ()) <$>
      genWhitespaces <*>
...
genArg :: MonadGen m => m (Expr '[] ()) -> m (Arg Expr '[] ())
genArg genExpr =
  sizedRecursive
...
genCompFor :: MonadGen m => m (CompFor Expr '[] ())
genCompFor =
  sized2M
    (\a b ->
...
genCompIf :: MonadGen m => m (CompIf Expr '[] ())
genCompIf =
  CompIf (Ann ()) <$>
  genWhitespaces <*>
  Gen.scale (max 0 . subtract 1) genExpr

genComprehension :: MonadGen m => m (Expr '[] ()) -> m (Comprehension VIdentity Expr '[] ())
genComprehension me =
  sized3
    (Comprehension (Ann ()))
    (VIdentity <$> me)
...
genSubscript :: MonadGen m => m (SubscriptItem Expr '[] ())
genSubscript =
  sizedRecursive
...
genDictComp :: MonadGen m => m (Comprehension DictItem Expr '[] ())
genDictComp =
  sized3
    (Comprehension (Ann ()))
    (genDictItem genExpr)
...
genSetComp :: MonadGen m => m (Comprehension SetItem Expr '[] ())
genSetComp =
  sized3
    (Comprehension (Ann ()))
...
genExpr' isExp =
  sizedRecursive
    ...
    , fmap VIn $
      ...
      genSetComp <*>
      genWhitespaces
    , VIn . Generator (Ann ()) <$> genComprehension genExpr
    , Gen.subtermM
        genExpr
        (\a -> fmap VIn $ Await (Ann ()) <$> genWhitespaces <*> pure a)
    , Gen.subtermM
        genExpr
        (\a ->
            fmap VIn $
    ...
    , sized2M
        (\a b ->
           (\ws1 ws2 -> VIn $ Call (Ann ()) a ws1 b ws2) <$>
           genWhitespaces <*>
           genWhitespaces)
        genExpr
    ...
    , sized2M
        (\a b ->
           (\ws1 ws2 -> VIn $ Subscript (Ann ()) a ws1 b ws2) <$>
           genWhitespaces <*>
           genWhitespaces)
        genExpr
    ...
    , sizedBind genExpr $ \a -> do
        ...
        sizedBind (genExpr' $ case op of; Exp{} -> True; _ -> False) $ \b ->
          pure . VIn $ Binary (Ann ()) a op b
    , Gen.subtermM
        (genExpr' isExp)
        (\a -> fmap VIn $ Parens (Ann ()) <$> genWhitespaces <*> pure a <*> genWhitespaces)
    , genTuple genExpr
    , fmap VIn $ Not (Ann ()) <$> genWhitespaces <*> genExpr
    , fmap VIn $ Unary (Ann ()) <$> genUnOp <*> genExpr
    , sized3M
        (\a b c ->
           (\ws1 ws2 -> VIn $ Ternary (Ann ()) a ws1 b ws2 c) <$>
           genWhitespaces <*>
           genWhitespaces)
        genExpr
        genExpr
        genExpr
    , fmap VIn $ Yield (Ann ()) <$> genWhitespaces <*> sizedCommaSep genExpr
    , fmap VIn $ YieldFrom (Ann ()) <$> genWhitespaces <*> genWhitespaces <*> genExpr
    , Gen.subtermM
        genExpr
        (\a ->
           fmap VIn $
