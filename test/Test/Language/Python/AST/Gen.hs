...
genStarExpr cfg =
  AST.StarExpr <$>
  genWhitespaceBeforeF
    (Gen.small . genExpr $ cfg & atomType .~ AST.SAssignable) <*>
  pure ()
...
genYieldArg cfg =
  case cfg ^. atomType of
    AST.SAssignable -> Gen.small $ Gen.choice [ yieldArgList cfg ]
    AST.SNotAssignable ->
      Gen.small $
      Gen.choice
        [ yieldArgList cfg
...
genYieldExpr
  :: MonadGen m
  => SyntaxConfig 'AST.NotAssignable cfg
  -> m (AST.YieldExpr ())
genYieldExpr cfg =
  Gen.small $
  AST.YieldExpr <$>
...
genDictItem
  :: MonadGen m
  => SyntaxConfig 'AST.NotAssignable cfg
  -> m (AST.DictItem 'AST.NotAssignable cfg ())
genDictItem cfg =
  AST.DictItem <$>
  genTest cfg <*>
...
genDictUnpacking
  :: MonadGen m
  => SyntaxConfig 'AST.NotAssignable cfg
  -> m (AST.DictUnpacking 'AST.NotAssignable cfg ())
...
genDictOrSetMaker
  :: MonadGen m
  => SyntaxConfig 'AST.NotAssignable ctxt
  -> m (AST.DictOrSetMaker 'AST.NotAssignable ctxt ())
...
genLambdefNocond
  :: MonadGen m
  => SyntaxConfig 'AST.NotAssignable ctxt
  -> m (AST.LambdefNocond 'AST.NotAssignable ctxt ())
