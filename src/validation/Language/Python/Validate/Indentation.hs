...
validateParamsIndentation
  ...
  -> ValidateIndentation e (CommaSep (Param Expr (Nub (Indentation ': v)) a))
validateParamsIndentation e = pure $ unsafeCoerce e

validateArgsIndentation
  :: AsIndentationError e a
  => CommaSep (Arg Expr v a)
  -> ValidateIndentation e (CommaSep (Arg Expr (Nub (Indentation ': v)) a))
...
validateCompoundStatementIndentation (If a idnt ws1 expr s elifs body1) =
  ...
  traverse
    (\(idnt2, x, y, z) ->
       ...
       checkIndent idnt2 <*>
       pure x <*>
       validateExprIndentation y <*>
       validateSuiteIndentation idnt z)
    elifs <*>
  traverse
    (\(idnt2, x, y) ->
       (,,) <$
       ...
       checkIndent idnt2 <*>
       pure x <*>
       validateSuiteIndentation idnt y)
    body1
...
validateCompoundStatementIndentation (TryExcept a idnt b c d e f) =
  ...
  traverse
    (\(x, y, z, w) ->
       ...
       setNextIndent EqualTo (idnt ^. indentsValue) <*>
       checkIndent x <*>
       traverse validateExceptAsIndentation z <*>
       validateSuiteIndentation idnt w)
    d <*
  setNextIndent EqualTo (idnt ^. indentsValue) <*>
  traverse
    (\(idnt2, x, y) ->
