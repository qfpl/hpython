...
orExpr ws =
  ...
  where
    ...
    factor =
      ((\(tk, s) -> let ann = pyTokenAnn tk in Unary ann (Negate (Ann ann) s)) <$>
       token ws (\case; TkMinus{} -> True; _ -> False) "-"
       <|>
       (\(tk, s) -> let ann = pyTokenAnn tk in Unary ann (Positive (Ann ann) s)) <$>
       token ws (\case; TkPlus{} -> True; _ -> False) "+"
       <|>
       (\(tk, s) -> let ann = pyTokenAnn tk in Unary ann (Complement (Ann ann) s)) <$>
    ...
    power =
      (\a -> maybe a (uncurry $ Binary (a ^. exprAnn) a)) <$>
      atomExpr <*>
